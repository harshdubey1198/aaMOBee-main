...
roleController.getDataFSuper = async (req,res) => {
    try {
        const data = await roleServices.getDataFSuper(req.params.id)
        return res.status(200).json(createResult("Data Fetched Succefully", data))
    } catch (error) {
...
roleController.getData = async (req,res) => {
    try {
