...
const sendDuePaymentNotification = async (req, res) => {
  try {
    await notificationService.triggerDuePaymentNotification();
    res.status(200).json({ message: "Due payment notifications triggered." });
  } catch (error) {
    console.error("Error in sendDuePaymentNotification:", error);
    res.status(500).json({ error: "Failed to trigger due payment notifications." });
  }
};
module.exports = { sendNotification, getUserNotifications, markNotificationAsRead, sendDuePaymentNotification };
