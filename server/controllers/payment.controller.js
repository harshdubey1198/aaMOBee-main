...
paymentController.createFreePlanPayment = async (req, res) => {
  try {
    const response = await PaymentService.createFreePlanPayment(req.body);
    return res.status(200).json(createResult("Free plan activated successfully", response));
  } catch (error) {
    console.error("Error activating free plan:", error.message || error);
    return res.status(500).json(createResult(null, null, error.message));
  }
};
