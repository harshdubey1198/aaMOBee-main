...
authController.setUserPermissions = async (req, res) => {
  try {
    const { id } = req.params;
    const updatedUser = await authService.setPermissionsForUser(id, req.body.sidebarAccess);
    return res.status(200).json(createResult("Permissions updated successfully", updatedUser, false));
  } catch (error) {
...
authController.getUserSidebar = async (req, res) => {
  try {
    const { id } = req.params;
    const sidebar = await authService.getSidebarForUser(id);
    return res.status(200).json(createResult("Sidebar fetched successfully", sidebar, false));
  } catch (error) {
    console.log("Get sidebar error:", error);
    return res.status(400).json(createResult(error.message, null, true));
  }
};

authController.updateUserPermissions = async (req, res) => {
  try {
    const { id } = req.params;
    const updatedUser = await authService.updateUserPermissions(id, req.body.sidebarAccess);
    return res.status(200).json(createResult("Permissions updated successfully", updatedUser, false));
  } catch (error) {
