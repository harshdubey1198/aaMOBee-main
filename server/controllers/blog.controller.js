...
blogController.getAllContactMessages = async (req, res) => {
    try {
      const { page = 1, limit = 10 } = req.query; // Default page 1, limit 10
  
      const result = await BlogServices.getContactMessagesPaginated(Number(page), Number(limit));
      return res.status(200).json(createResult('Contact messages fetched successfully', result));
    } catch (error) {
