...
ManufacturerServices.createManufacturer = async (userId, body) => {
    ...
    const { name, contactPerson, phone, email, address, firmId: manualFirmId } = body;
  
    const firmIdToUse = user.role === "client_admin" ? manualFirmId : user.adminId;
  
    if (!firmIdToUse) {
      throw new Error("Firm ID is required.");
    }
  
    const existingVendor = await Manufacturer.findOne({ name, email, firmId: firmIdToUse });
    if (existingVendor) {
