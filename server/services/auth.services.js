...
authService.userLogin = async (body) => {
    try {
        ...
        if(user.adminId){
            const Adminfirm = await User.findOne({ _id: user.adminId });
        }
        let clientAdmin;
        if (['firm_admin', 'employee', 'accountant'].includes(user.role)) {
            const firmAdmin = await User.findOne({ _id: user.adminId });
            console.log('====================================');
            console.log(firmAdmin);
            console.log('====================================');
             // ❌ If firmAdmin (firm) not found or soft deleted, deny login
...
//         if (!latestPayment) {
//             if (user.role === 'client_admin') {
//                 throw new Error("No active subscription found. Please purchase or renew a plan to continue.");
//             } else {
//                 throw new Error("Your account cannot be accessed due to an inactive subscription. Please contact your Client Admin.");
//             }
//         }
//         if (new Date() > new Date(latestPayment.expirationDate)) {
...
//             if (user.role === 'firm_admin' || user.role === 'employee' || user.role === 'accountant') {
//                 throw new Error("Your account cannot be accessed at the moment. Please contact your Client Admin.");
//             } else {
...
authService.getCompany = async (id) => {
    try {
        const data = await User.find({ adminId: id , isDeleted:false }).select("-password").populate("adminId");
...
authService.getSidebarForUser = async (userId) => {
    ...
    const sortedSidebar = (user.sidebarAccess || [])
        ...
        .map(item => ({
            ...item.toObject(),
            subItem: (item.subItem || []).sort((a, b) => (a.order || 0) - (b.order || 0))
        }));
