...
BomServices.createbom = async (body) => {
  const { 
    productName, rawMaterials, firmId, createdBy, 
    manufacturer, brand, type, sellingPrice, 
    categoryId, subcategoryId = null, vendor, 
    taxId, selectedTaxTypes, ProductHsn, qtyType 
  } = body;
  ...
  // ✅ self-fallback logic for vendor / brand / manufacturer
  let vendorToUse = vendor;
  let brandToUse = brand;
  let manufacturerToUse = manufacturer;

  if (vendor === "self") {
    vendorToUse = firmId; // store firmId reference
  }
  if (brand === "self") {
    brandToUse = firmId; // store firmId reference
  }
  if (manufacturer === "self") {
    manufacturerToUse = firmId; // store firmId reference
  }
...
BomServices.getboms = async (body) => {
  ...
  const data = await BOM.find({ firmId, deleted_at: null })
    ...
    .populate({ path: "manufacturer", select: "name companyTitle" })
    .populate({ path: "brand", select: "name companyTitle" })
    .populate({ path: "vendor", select: "name companyTitle" })
  ...
  for (const bom of data) {
    ...
    // unify brand/vendor/manufacturer → return name if exists, else firmTitle
    if (bom.brand) {
      bom.brandName = bom.brand.name || bom.brand.companyTitle || "N/A";
    }
    if (bom.vendor) {
      bom.vendorName = bom.vendor.name || bom.vendor.companyTitle || "N/A";
    }
    if (bom.manufacturer) {
      bom.manufacturerName = bom.manufacturer.name || bom.manufacturer.companyTitle || "N/A";
    }
  }
...
BomServices.getbomById = async (bomId) => {
  ...
  if (!data) throw new Error("No BOM found");

  if (data.tax.selectedTaxTypes.length > 0 && data.tax.taxId?.taxRates) {
    data.tax.selectedTaxTypes = data.tax.taxId.taxRates.filter(rate =>
      data.tax.selectedTaxTypes.some(id => id.toString() === rate._id.toString())
    );
  }

  // unify brand/vendor/manufacturer
  if (data.brand) {
    data.brandName = data.brand.name || data.brand.companyTitle || "N/A";
  }
  if (data.vendor) {
    data.vendorName = data.vendor.name || data.vendor.companyTitle || "N/A";
  }
  if (data.manufacturer) {
    data.manufacturerName = data.manufacturer.name || data.manufacturer.companyTitle || "N/A";
  }
