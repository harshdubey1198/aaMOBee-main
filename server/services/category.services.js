...
categoryServices.createCategory = async (userId, body) => {
    const { categoryName, parentId, description, firmId: payloadFirmId } = body;
    ...
    const existingCategory = await Category.findOne({ categoryName, firmId: firmIdToUse });
    if (existingCategory) {
      throw new Error('Category with this name already exists for this firm');
    }
