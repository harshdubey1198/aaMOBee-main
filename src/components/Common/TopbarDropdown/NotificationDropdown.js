...
const NotificationDropdown = (props) => {
  ...
  useEffect(() => {
    ...
    socket.on("duePaymentAlert", (data) => {
      const newNotification = {
        _id: Date.now().toString(), // Fallback unique ID
 ...
 setNotifications((prev) => {
    ...
    return () => {
      ...
      socket.off("duePaymentAlert");   // Clean up this listener too
      socket.disconnect();
    };
  }, [userId]);
              ...
              >
                <div className="d-flex">
                  ...
                  <div className="flex-grow-1">
                    <h6 className="mb-1">
                      {notif.message.toLowerCase().includes("payment due") ? (
                        <strong style={{ color: "#c0392b" }}>{notif.message}</strong>
                      ) : (
                        notif.message
                      )}
                    </h6>
