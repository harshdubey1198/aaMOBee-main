...
const getCurrencyDetails = (currencyCode) => {
  ...
  return currency ? currency.code : currencyCode; // Returns only the symbol
};
...
console.log("currency", currency);
    ...
    const grandTotal = subtotal + totalTaxAmount; // Amount after tax
    const finalNetAmount = grandTotal - totalDiscount; // Final amount after discount
      ...
      <div ref={ref} className="card p-4 border rounded position-relative">
        ...
        <div className="row m-text-center p-2 pt-3 pb-0 m-0">
          <div className="col-lg-6 col-md-6 col-sm-12 mb-4">
            ...
            {(invoiceData?.firmId?.registeredTaxationDetail?.find(item => item.toShow)?.fieldValue ||
              selectInvoice?.firmId?.registeredTaxationDetail?.find(item => item.toShow)?.fieldValue) && (
              <p className="my-1">
                <b>{invoiceData?.firmId?.registeredTaxationDetail?.find(item => item.toShow)?.fieldName || 
                    selectInvoice?.firmId?.registeredTaxationDetail?.find(item => item.toShow)?.fieldName}:</b>{" "}
                {invoiceData?.firmId?.registeredTaxationDetail?.find(item => item.toShow)?.fieldValue || 
                selectInvoice?.firmId?.registeredTaxationDetail?.find(item => item.toShow)?.fieldValue}
              </p>
            )}
          ...
          <div className="col-lg-6 col-md-6 col-sm-12 m-text-center text-end">
            ...
            <p><strong>Amount Due:</strong> {invoiceData.invoiceCurrency || currency || "INR"} {invoiceData?.amountDue}</p>
        ...
        <div className="row p-1 pt-0 m-0 border-bottom">
          <div className="col-lg-6 col-md-6 col-sm-12 m-text-center">
            ...
            <p className="my-1">
              Phone : {invoiceData?.customerPhone} <br/> Email :{" "}
        ...
        <div className="row bg-light p-4 m-0">
          ...
          <div className="col-lg-6 col-md-6 col-sm-12 m-text-center text-end">
            ...
            {/* <p className="my-1"><strong>Subtotal (Amount before Tax):</strong> {invoiceData.invoiceCurrency || currency || "INR"} {subtotal.toFixed(2)}</p>
            <p className="my-1"><strong>Total Tax Amount:</strong> {invoiceData.invoiceCurrency || currency || "INR"} {totalTaxAmount.toFixed(2)}</p>
            <p className="my-1"><strong>Grand Total (After Tax):</strong> {invoiceData.invoiceCurrency || currency || "INR"} {grandTotal.toFixed(2)}</p>
            <p className="my-1"><strong>Amount Paid:</strong> {invoiceData.invoiceCurrency || currency || "INR"} {invoiceData?.amountPaid.toFixed(2)}</p>
            <p className="my-1"><strong>Total:</strong> {invoiceData.invoiceCurrency || currency || "INR"} {totalAmount.toFixed(2)}</p> */}
            <h5>Payment Summary</h5>
            <p className="my-1"><strong>Amount After Tax:</strong> {invoiceData.invoiceCurrency || currency || "INR"} {grandTotal.toFixed(2)}</p>
            <p className="my-1"><strong>Total Discount:</strong> {invoiceData.invoiceCurrency || currency || "INR"} {totalDiscount.toFixed(2)}</p>
            <p className="my-1"><strong>Final Net Amount:</strong> {invoiceData.invoiceCurrency || currency || "INR"} {finalNetAmount.toFixed(2)}</p>
            <p className="my-1"><strong>Amount Paid:</strong> {invoiceData.invoiceCurrency || currency || "INR"} {invoiceData?.amountPaid.toFixed(2)}</p>


            <p className="my-1 value-in-words">
              <strong>Total in Words:</strong> {convertNumberToWords(Number(totalAmount.toFixed(2)))} {invoiceData.invoiceCurrency || currency || "INR"} Only
            </p>
