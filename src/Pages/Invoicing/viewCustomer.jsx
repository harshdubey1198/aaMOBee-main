...
const ViewCustomer = () => {
    ...
    useEffect(() => {
        const fetchCustomers = async (firmId) => {
            try {
                const idToUse = role === "client_admin" ? selectedFirmId : firmId;
                if (!idToUse) return;
                const response = await getCustomers(`${idToUse}`);
            ...
            <div className='page-content'>
                ...
                <div className="d-flex flex-wrap justify-content-start align-items-center gap-2 mb-3">
                     ...
                     {role === "client_admin" && (
                      ...
                      <i className='bx bx-refresh cursor-pointer'  style={{fontSize: "24.5px",fontWeight: "bold",color: "black",transition: "color 0.3s ease"}} onClick={handleRefetch} onMouseEnter={(e) => e.target.style.color = "green"}  onMouseLeave={(e) => e.target.style.color = "black"}></i>
                </div>
