...
import { Modal, ModalHeader, ModalBody, ModalFooter, Button, Input, Label, Card, CardBody, Row, Col, FormGroup } from 'reactstrap';
...
const handleAssignTask = async () => {
  ...
  return (
    <React.Fragment>
              ...
              <div className='card'>
                <div className='card-body'>
                  ...
                  <div className='button-panel'>
                      ...
                      <i className='bx bx-refresh cursor-pointer'  style={{fontSize: "24.5px",fontWeight: "bold",marginRight: "10px",color: "black",transition: "color 0.3s ease"}} onClick={handleRefetchTasks} onMouseEnter={(e) => e.target.style.color = "green"}  onMouseLeave={(e) => e.target.style.color = "black"}></i>
                  </div>
                              ...
                              <tr
                                  ...
                                  style={{
                                    backgroundColor: new Date(task.dueDate) < new Date() && task.status !== "completed"
                                      ? "#ffe6e6"  // red-pink shade for missed
                                      : getLeadColor(task.leadIds),
                                    cursor: new Date(task.dueDate) < new Date() && task.status !== "completed"
                                      ? "not-allowed"
                                      : "pointer",
                                    opacity: new Date(task.dueDate) < new Date() && task.status !== "completed" ? 0.7 : 1
                                  }}
                                  onClick={() => {
                                    const isMissed = new Date(task.dueDate) < new Date() && task.status !== "completed";
                                    if (!isMissed) {
                                      setLeadMainDetailModal(true);
                                      setSelectedLead(lead);
                                    }
                                  }}
                                >
                                ...
                                <td onClick={
                                  ...
                                  <button
                                      ...
                                      disabled={new Date(task.dueDate) < new Date() && task.status !== "completed"}
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        const isMissed = new Date(task.dueDate) < new Date() && task.status !== "completed";
                                        if (!isMissed) handleUpdate(task, lead);
                                      }}
                                    >
                                      Update
                                    </button>
      ...
      <Modal isOpen={leadMainDetailModal} toggle={toggleLeadMainDetailModal} size="lg">
          <ModalHeader toggle={toggleLeadMainDetailModal}>Lead Details</ModalHeader>
          <ModalBody>
            {selectedLead && (
              <Card className="shadow glass-card border-0 mb-3">
                <CardBody>
                  <h4 className="mb-4 text-primary fw-bold">Lead Overview</h4>
                  ...
                  <Row>
                    {Object.entries(selectedLead)
                      .filter(
                        ([key, value]) =>
                          ![
                            "mode", "_id", "additionalFields", "createdAt", "updatedAt",
                            "createdBy", "updatedBy","notes", "deleted_at", "firmId", "deletedAt","additionalFields","assignmentHistory",
                            "deletedBy", "__v", "remarks", "leadIds", "assignedTo", "assignedBy", "dueDate"
                          ].includes(key) &&
                          value !== null &&
                          value !== undefined &&
                          value !== ""
                        ...
                        <FormGroup>
                          ...
                          >
                            ...
                            selectedLead.assignmentHistory.length > 0 ? (
                              selectedLead.assignmentHistory.map((assignment) => (
                                <div key={assignment._id} style={{ marginBottom: "10px" }}>
                                  <div className="d-flex justify-content-between">
                                    <strong style={{ width: "40%", paddingLeft: "30px" }}>
                                      {assignment.assignedBy?.roleId?.roleName .split("_") .map((part) => part.charAt(0).toUpperCase() + part.slice(1)) .join(" ") || assignment.assignedBy?.role .split("_") .map((part) => part.charAt(0).toUpperCase() + part.slice(1)) .join(" ") } -{" "}
                                    ...
                                    <strong style={{ width: "40%", paddingLeft: "30px" }}>
                                      {assignment.assignedTo?.roleId?.roleName .split("_").map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" ")} -{" "}
